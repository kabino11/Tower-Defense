...
Game.input = (function() {
	...
	function Keyboard() {
		...
		//removes handlers based on input function (useful for clearing out key assignments for action)
		that.deregisterCommand = function(toRemove) {
			for(handler = that.handlers.length - 1; handler >= 0; handler--) {
				if(that.handlers[handler].handler == toRemove) {
					that.handlers.splice(handler, 1);
				}
			}
		}
	...
	function Mouse() {
		...
		window.addEventListener('mousemove', function(event) {
			lastPosition = getMousePos(canvas, event);
		});
